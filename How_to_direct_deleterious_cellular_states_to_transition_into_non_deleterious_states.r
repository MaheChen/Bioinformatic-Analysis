---
title: 'How to direct deleterious cellular states to transition into non-deleterious states'
...
```{r, echo=FALSE, fig.width=12, fig.height=5}
h1 <- drug1_dose5_t4 %>% ggplot(aes(x = NGFR)) + geom_boxplot(fill="red") + xlim(2.5, 4.5)
h2 <- drug1_dose5_t4 %>% ggplot(aes(x = MiTFg)) + geom_boxplot(fill="yellow") + xlim(2.5, 4.5)
h3 <- drug1_dose5_t4 %>% ggplot(aes(x = AXL)) + geom_boxplot(fill="green") + xlim(2.5, 4.5)
h4 <- drug1_dose5_t4 %>% ggplot(aes(x = Sox10)) + geom_boxplot(fill="blue") + xlim(2.5, 4.5)
